...
public interface ComplaintsMessagesRepository extends JpaRepository<ComplaintsMessages, Integer> {
    ...
    @Query(value = "SELECT distinct *  from complaints_messages where complain_id =?1 " +
            " limit ?2 offset ?3  ", nativeQuery = true)
    List<ComplaintsMessages> findAllByComplaintIdWithPagination(int cId , int limit,int offset);
